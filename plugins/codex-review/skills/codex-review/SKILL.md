...
**Start session** - include in your first request to the Codex MCP server's `codex` tool:
```
...
**Continue session** - use the Codex MCP server's `codex-reply` tool with the SESSION_ID.
